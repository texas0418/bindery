// src/proAccess.ts
// Fail-open entry gate for The Bindery. The one product: the full Unwriting
// restoration. Where the gate falls is an OPEN DESIGN DECISION (see README —
// the open book, doctrine 10b, means pages can't be door-gated; the gate
// must be on solving depth, e.g. the first N solves free), so nothing calls
// useEntryUnlocked yet.
//
...
// placeholder, the entry is UNLOCKED. Never hide the story behind a wall the
...
export async function purchaseEntry(): Promise<boolean> {
  const Purchases = getPurchases();
...
export async function restoreEntry(): Promise<boolean> {
  const Purchases = getPurchases();
  if (!Purchases || isPlaceholderKey(keyForPlatform())) {
    setUnlocked(true);
    return true;
...
export const useEntryUnlocked = (): boolean =>
